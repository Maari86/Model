...
public class Lighsaber : MonoBehaviour
{
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        // Slice the object and destroy the original
        GameObject[] slices = Slicer.Slice(plane, other.gameObject);
        Destroy(other.gameObject);

        // Apply equal force to all slices
        foreach (GameObject slice in slices)
        {
            Rigidbody rigidbody = slice.GetComponent<Rigidbody>();
            Vector3 forceDirection = new Vector3(0.08f * (Random.Range(0f,1f) < 0.5f ? 1:-1), 0.15f, 0.3f); // Upward and forward direction in world space
            rigidbody.AddForce(forceDirection * _forceAppliedToCut, ForceMode.Impulse);
        ...
        if (slices.Length > 0)
        {
            int feedbackLevelIndex = other.gameObject.GetComponent<CubeScript>().feedbackLevel; // Calculate the feedback level index based on cut quality
            feedbackManager.DisplayFeedbackMessage(feedbackLevelIndex);
            scoreCounter.IncreaseScore(feedbackLevelIndex); // Pass the feedbackLevelIndex as an argument
        }
        else
        {
            feedbackManager.DisplayFeedbackMessage(-1); // Display missed feedback
        }
    }
